...
def predict_remarks(products):
    dicts = {
        'Furnitures': ['CHAIR', 'DINING', 'SHOWCASE', 'BED', 'SOFA', 'LOW-BED', 'DARAJ', 'PALANG', 'DRESSING', 'TABLE', 'CORNER', 'SOFA'],
        'Electronics': ['FRIDGE', 'VACUUM', 'OVEN', 'MIXER', 'GRINDER', 'COOKER', 'FRYER', 'ELECTRIC', 'JUG', 'JAR', 'BALTRA', 'LED', 'BULB', 'HEATER', 'FOSTER', 'INDUCTION'],
    ...
    for category, keywords in dicts.items():
        dicts[category] = [keyword.lower() for keyword in keywords]

    # Create a dictionary to store category counts
    category_counts = {category: 0 for category in dicts}

    for product in products:
        words = product.upper().split(' ')  # Convert words to uppercase for case-insensitive comparison
        for word in words:
            for category, keywords in dicts.items():
                if word.lower() in keywords:  # Check for matches using lowercase comparison
                    category_counts[category] += 1
                    break  # Break the inner loop if the category is found

    # Get the most frequent category based on counts
    predicted = max(category_counts, key=category_counts.get)

    # Calculate the count of "Unknown Item" category
    unknown_count = category_counts.get('Unknown Item', 0)

    # Reconsider the threshold based on your specific use case
    threshold = 100

    if category_counts[predicted] + threshold > unknown_count:
        return predicted
    else:
        return 'Unknown Item'
